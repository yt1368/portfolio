...
//(1)生成商品列、(2)商品加入購物車的事件監聽、(3)商品加入購物車後跳出"成功加入購物車"的訊息
    ...
    addProduct.forEach((addToCartButton)=>                                 
        {                                
            addToCartButton.addEventListener("click",function(event)    
            {
                ...
                event.preventDefault();                                 //preventDefault()阻止預設行為(連結到各商品的介紹頁面)
...
//(電腦版)使用者搜尋後，跳轉到搜尋結果頁面(search.html)(生成搜尋結果頁面的程式碼在search.js中)
...
//(手機板)使用者搜尋後，跳轉到搜尋結果頁面(search.html)(生成搜尋結果頁面的程式碼在search.js中)
export function phoneSearchResult(){
    let searchBtn = document.querySelector(".phone_user_search_img");        
    searchBtn.addEventListener("click", function()                       
    {
        let userSearch = document.querySelector(".phone_user_search").value;     
        location.href=`search.html?input=${userSearch}`;                   
    })
}