...
import { searchResult, phoneSearchResult } from"./menu-product-list.js";
...
export function presentCheckoutProductList()        
{
    ...
    checkoutBuyNum.forEach((productNum)=>
    {
        productNum.addEventListener("change", function()                        
        {
            ...
            presentCheckoutAmountList();            // updateProductNumToCart()，()中第一個值指的是商品的id，()中第二個值指的是使用者輸入的商品數量(但因為productNum.value的資料型態是字串，因此要先轉換成"數字"的資料型態)
            searchResult();                         // 一開始的value="${item.num}"(上面生成結帳頁面html的設定)，而這邊productNum.value指的是 change事件監聽器後執行的函式，也就是使用者在輸入框修改後的值
            phoneSearchResult();
        })                      
    })                          
...
//在結帳頁面中，生成運送方式的HTML
    function generateDeliveryOptions(productId,deliveryId)    //()中的productId指的是上面在呼叫此函式時，所回傳的cartItem.id(商品id)，而deliveryId指的是item.deliveryId(商品預設的運送方式)
    {
        let deliveryHTML="";
